...
class Client extends discord.Client {
    ...
    getUser = (id, message, i = 0) =>
        !id
            ? null
            : message?.mentions.users.map((e) => e)[i] ||
              message?.guild.members.cache.get(id)?.user ||
              message?.guild.members.cache.find(
                  (e) =>
                      e.user.username.toLowerCase().search(id?.toLowerCase()) >=
                      0
              )?.user ||
              this.users.cache.get(id) ||
              this.users.cache.find(
                  (e) => e.username.toLowerCase().search(id?.toLowerCase()) >= 0
              ) ||
              this.users.fetch(id).catch((e) => undefined);
    ...
    async fetch(url, method, body, timeout = 5000) {
        return await this.funcs.fetchWithTimeout(
            url,
            {
                method,
                headers: {
    ...
    init() {
        this.login(this.options.token).then(() => {
            this.application.commands.set(
                this.slashCommands.map((e) => e.data.toJSON())
            );
        });
